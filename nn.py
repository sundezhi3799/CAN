...
def conv(x, filter_size, num_filters, stride, weight_decay, name, padding='SAME', groups=1, trainable=True):
    ...
    with tf.variable_scope(name):
        ...
        biases = tf.get_variable('b', shape=[num_filters], trainable=trainable,
                                 initializer=tf.keras.initializers.glorot_normal())
...
def fc(x, num_out, weight_decay, name, relu=True, trainable=True):
    ...
    with tf.variable_scope(name):
        ...
        weights = tf.get_variable('W',
                                  ...
                                  regularizer=regularizer,
                                  collections=['variables'])
        biases = tf.get_variable('b', [num_out], initializer=tf.keras.initializers.glorot_normal(), trainable=trainable)
...
def validate(sess, model, x_test, y_test, batch_size):
    ...
    for i in range(int(x_test.shape[0] / batch_size)):
        val_acc, val_loss = model.validate(sess, x_test[i * batch_size:(i + 1) * batch_size],
                                           y_test[i * batch_size:(i + 1) * batch_size])
        accuracy = accuracy + val_acc
        # accuracy=accuracy+model.validate(sess, x_test[i * batch_size:(i + 1) * batch_size],
        #                                                   y_test[i * batch_size:(i + 1) * batch_size])
        losses.append(val_loss)
    # Tail Case
    if x_test.shape[0] % batch_size != 0:
        val_acc, val_loss = model.validate(sess, x_test[(i + 1) * batch_size:],
                                           y_test[(i + 1) * batch_size:])
        accuracy += val_acc
        # accuracy+=model.validate(sess, x_test[(i + 1) * batch_size:],
        #                                                   y_test[(i + 1) * batch_size:])
        losses.append(val_loss)
    return accuracy / x_test.shape[0], np.mean(losses)
...
def train(sess, model, x_train, y_train, x_test, y_test, epochs, batch_size, summary_writer=0, train_validation=5):
    ...
    for i in range(epochs):
        # v9 = sess.graph.get_tensor_by_name('network/fc9/W:0')
        # print(sess.run(v9))
        # Shuffle indicies
        ...
        if x_test is not None:
            val_acc, val_loss = validate(sess, model, x_test, y_test, batch_size)
            ...
            print("Epoch {:1}|".format(i),
                  ...
                  "Val Loss: {:.4}|".format(val_loss),
                  "Iter {}/{}|".format(x_train.shape[0], x_train.shape[0]),
                  "Time {:1.2}".format(stop - start),
                  'lr {}'.format(model.learning_rate),
                  'weight_decay {}'.format(model.weight_decay),
                  '\r')
            history["Epoch"].append(i)
            history["Iter"].append(j)
            history["Loss"].append(np.mean(losses))
            history["Training Acc"].append(np.mean(train_accs))
...
def prediction(sess, model, x_test, y_test, train_lst, val_lst, batch_size=32):
    ...
    for i in range(int(X_val.shape[0] / batch_size)):
        ...
        y_preds[batch_size * i:batch_size * (i + 1)] = np.argmax(Y_val[batch_size * i:batch_size * (i + 1)],
                                                                 axis=1).astype('uint8')
