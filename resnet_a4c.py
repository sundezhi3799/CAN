# example: python resnet_a4c.py -i 'segmented-images-all' -l 'train_test_pah_a4c_plax_all.csv'
...
class Network(object):
    ...
    def train(self, out_path, learning_rate, weight_decay, momentum, epochs, batch_size, x_train, y_train, x_test,
              ...
              save_best=True):
        # If there is a lift, it will be covered once
    ...
    for sample_index in [train_index, test_index]:
        ...
        for study in sample_index:
            ...
            if y:
                ...
                positive_study += 1
            a4c = eval(labels_file.loc[study, 'a4c'])
            for image_name in a4c:
...
def main(image_dir, labels_file, label_dim, feature_dim, batch_size, epochs, learning_rate, weight_decay, momentum,
    ...
    out_path = "model/pah_2_a4c_%s_%s.hdf5" % (model_name, timenow)
    ...
    json.dump(par_dct, open('parameter//pah_2_a4c_%s_%s.json' % (model_name, timenow), 'w+'))
    if not save_best:
        os.makedirs('model', exist_ok=True)
        network.model.save_weights("model/pah_2_a4c_%s_%s.hdf5" % (model_name, timenow))
        print("model/pah_2_a4c_%s_%s.hdf5" % (model_name, timenow))
    os.makedirs('history', exist_ok=True)
    with open("history/pah_2_a4c_%s_%s.pickle" % (model_name, timenow), 'wb') as file_pi:
