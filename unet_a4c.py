...
def main(image_dir, lables_dir, label_dim=5, feature_dim=3, batch_size=64, epochs=100, lr=1e-3, wd=5e-4, momentum=0.9,
         drop_out=0.5):
    early_stop = False
    reducing_lr = False
    model_name = 'unet'
    ...
    network.model.save_weights("model/unet_a4c_%s_%s.hdf5" % (model_name, timenow))
    os.makedirs('history', exist_ok=True)
    with open("history/unet_a4c_%s_%s.pickle" % (model_name, timenow), 'wb') as file_pi:
