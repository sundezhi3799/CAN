...
class Network(object):
    def __init__(self, model_name='unet', feature_dim=1, label_dim=5, dropout=0.5):
    ...
    def unet(self):
        ...
        conv10 = Conv2D(self.label_dim, 1, activation='softmax')(conv9)
    ...
    def train(self, learning_rate, weight_decay, momentum, epochs, batch_size, x_train, y_train, x_test, y_test,
              loss='categorical_crossentropy', metrics=['accuracy'], early_stop=True, reducing_lr=True):
...
def load_train_test_data(image_dir, labels_dir, label_dim):
    ...
    for image_name in os.listdir(labels_dir):
        ...
        label = imread(os.path.join(labels_dir, label_name)).astype('float32')
...
def main(image_dir, lables_dir, label_dim=5, view='a4c', feature_dim=3, batch_size=64, epochs=100, lr=1e-3, wd=5e-4, momentum=0.9,
         drop_out=0.5):
    early_stop = False
    reducing_lr = False
    model_name = 'unet'
    ...
    network.model.save_weights("model/unet_%s_%s_%s.hdf5" % (view, model_name, timenow))
    os.makedirs('history', exist_ok=True)
    with open("history/unet_%s_%s_%s.pickle" % (view, model_name, timenow), 'wb') as file_pi:
...
if __name__ == '__main__':
    ...
    main(image_dir, label_dir, label_dim, view, feature_dim, batch_size, epochs, learning_rate, weight_decay, momentum,
         drop_out)
