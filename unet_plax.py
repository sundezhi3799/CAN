...
def main(image_dir, lables_dir, label_dim=4, feature_dim=3, batch_size=64, epochs=100, lr=1e-3, wd=5e-4, momentum=0.9,
         drop_out=0.5):
    early_stop = False
    reducing_lr = False
    model_name = 'unet'
    ...
    network.model.save_weights("model/unet_plax_%s_%s.hdf5" % (model_name, timenow))
    os.makedirs('history', exist_ok=True)
    with open("history/unet_plax_%s_%s.pickle" % (model_name, timenow), 'wb') as file_pi:
...
if __name__ == '__main__':
    ...
    parser.add_option("-n", "--label_dim", dest="label_dim", default=4, type='int')
